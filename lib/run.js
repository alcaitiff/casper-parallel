...
function getFiles(conf) {
  ...
  process.argv.forEach(function(val, index) {
    if (index > 1) {
      walkSync(val, files, conf);
    }
  });
  return files;
}

function validTestFile(fileName, conf) {
  return (
    fileName !== conf &&
    fileName.substr(fileName.length - 3, 3) === '.js'
  );
}

function walkSync(dir, filelist, conf) {
  filelist = filelist || [];
  if (fs.statSync(dir).isDirectory()) {
    ...
    files.forEach(function(file) {
      if (fs.statSync(path.join(dir, file)).isDirectory()) {
        filelist = walkSync(path.join(dir, file), filelist, conf);
      } else if (validTestFile(file, conf)) {
...
function startIt(colors, begin) {
  ...
  charm.cursor(false);
  //Print the reader
  p.printLine(lines.title, colors.info('Casper Parallel') + '\t' + begin.format(
    'DD/MM/YYYY HH:mm:ss'));
}
...
function bar() {
  ...
  var end = moment();
  var dur = moment.duration(end.diff(begin));
  var t = moment.utc(dur.asMilliseconds()).format("HH:mm:ss");
  p.printLine(lines.time, "Time:\t" + colors.time(t));
