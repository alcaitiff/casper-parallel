...
module.exports = {
  ...
  randomString: function(length, nolatin) {
    ...
    var chars = [vogals, consonants];
    var result = "";
    var use = Math.round(Math.random() * 10) % 2;
    for (var i = length; i > 0; --i) {
      result += chars[use][Math.round(Math.random() * (chars[use].length -
        1))];
      use = (use) ? 0 : 1;
    }
    return result;
  ...
  Elements: function(s, n, a) {
    ...
    for (var index = 0; index < a.size(); index++) {
      var selector = this.replaceInSelector(s, a[index]);
      r[index] = {
        "selector": selector,
        "name": n + " " + a[index]
      };
  ...
  replaceInSelector: function(s, v) {
    var stringSelector = (this.isXpath(s)) ? s.path : s;
    var newStringSelector = stringSelector.split("%s").join(v);
    if (this.isXpath(s)) {
      return {
        type: "xpath",
  ...
  concatInSelector: function(s, v) {
    var stringSelector = (this.isXpath(s)) ? s.path : s;
    var newStringSelector = stringSelector + v;
    if (this.isXpath(s)) {
      return {
        type: "xpath",
        path: newStringSelector
  ...
  Column: function(s) {
    ...
    this.asc = function(v) {
      return {
        "selector": this.concatInSelector(this.replaceInSelector(s, v),
          "[@class=\"order\"]"),
...
casper.fillAutoCompletion = function(element, text, index) {
  ...
  this.waitUntilVisible("[class*='ui-front']",
    function() {
